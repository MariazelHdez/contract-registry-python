...
async def download_report(page, from_year, to_year):
    ...
    await asyncio.sleep(60)  # Adjust based on download size or implement a more reliable download completion check
...
async def wait_for_page_ready(page, page_ready_event):
    ...
    try:
        ...
        await asyncio.wait_for(page_ready_event.wait(), timeout=10)
        print("La página está lista.")
    except asyncio.TimeoutError:
        print("Tiempo de espera excedido para la página. Revisando manualmente...")

        # **Verifica si la página realmente está cargada**
        loaded = await page.evaluate('document.readyState')
        if loaded == "complete":
            print("La página ya está completamente cargada.")
            page_ready_event.set()
        else:
            print("La página no ha terminado de cargar. Intentando recargar...")
            await page.reload({'waitUntil': 'networkidle2'})
